...
// reflectcall / CALLFN ABI0 frame offsets, packed at PtrSize=4.
// reflectcall(stackArgsType *_type, fn, stackArgs unsafe.Pointer,
//             stackArgsSize, stackRetOffset, frameSize uint32, regArgs *abi.RegArgs)
#define stackArgsType_off	0
#define fn_off			4
#define stackArgs_off		8
#define stackArgsSize_off	12
#define stackRetOffset_off	16
#define frameSize_off		20
#define REFLECTCALL_ARGSIZE	28
...
TEXT ·reflectcall(SB), NOSPLIT, $0-REFLECTCALL_ARGSIZE
	I64Load32U fn+fn_off(FP)
	I64Eqz
	If
		CALLNORESUME runtime·sigpanic<ABIInternal>(SB)
	End

	MOVW frameSize+frameSize_off(FP), R0
...
TEXT NAME(SB), WRAPPER, $MAXSIZE-REFLECTCALL_ARGSIZE; \
	NO_LOCAL_POINTERS; \
	MOVW stackArgsSize+stackArgsSize_off(FP), R0; \
	\
	Get R0; \
	I64Eqz; \
	Not; \
	If; \
		Get SP; \
		I64Load32U stackArgs+stackArgs_off(FP); \
		I32WrapI64; \
		I64Load32U stackArgsSize+stackArgsSize_off(FP); \
		I32WrapI64; \
		MemoryCopy; \
	End; \
	\
	MOVW f+fn_off(FP), CTXT; \
	Get CTXT; \
	I32WrapI64; \
	I64Load32U $0; \
	CALL; \
	\
	I64Load32U stackRetOffset+stackRetOffset_off(FP); \
	Set R0; \
	\
	MOVW stackArgsType+stackArgsType_off(FP), RET0; \
	\
	I64Load32U stackArgs+stackArgs_off(FP); \
	Get R0; \
	I64Add; \
...
// reflectcallmove(typ *_type, dst, src unsafe.Pointer, size uintptr, regs *abi.RegArgs)
// ABI0 arg offsets, packed at PtrSize.
#define rcm_typ		0
#define rcm_dst		4
#define rcm_src		8
#define rcm_size	12
...
TEXT callRet<>(SB), NOSPLIT, $40-0
	NO_LOCAL_POINTERS
	MOVW RET0, rcm_typ(SP)
	MOVW RET1, rcm_dst(SP)
	MOVW RET2, rcm_src(SP)
	MOVW RET3, rcm_size(SP)
	MOVW $0,   rcm_regs(SP)
...
TEXT gcWriteBarrier<>(SB), NOSPLIT, $0
	Loop
		...
		// Is there enough room? Subtract as i32 so an end value of
		// zero can represent the end of the 32-bit address space.
...
// gcWriteBarrierN requests space for N pointers (N*PtrSize bytes, PtrSize=4).
