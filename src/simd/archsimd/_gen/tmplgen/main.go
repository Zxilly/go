...
`, s, path.Base(splitRewritesPath))

	for _, p := range []string{splitCorePath, splitOpPath} {
		if p != splitRewritesPath {
...
func init() {
	...
	if splitPhase >= phase3Compile {
		splitRewritesPath = "cmd/compile/internal/ssacompile"
	}
...
func ternOpForLogical(out io.Writer) {
	fmt.Fprintf(out, `
func ternOpForLogical(op %[1]sOp) %[1]sOp {
	switch op {
`, splitOpPrefix)
	...
	intShapes.forAllShapes(func(seq int, t, upperT string, w, c int, out io.Writer) {
		...
		fmt.Fprintf(out, "case %[3]sOpAndInt%[1]dx%[2]d, %[3]sOpOrInt%[1]dx%[2]d, %[3]sOpXorInt%[1]dx%[2]d,%[3]sOpAndNotInt%[1]dx%[2]d: return %[3]sOpternInt%[4]dx%[5]d\n", w, c, splitOpPrefix, wt, ct)
		fmt.Fprintf(out, "case %[3]sOpAndUint%[1]dx%[2]d, %[3]sOpOrUint%[1]dx%[2]d, %[3]sOpXorUint%[1]dx%[2]d,%[3]sOpAndNotUint%[1]dx%[2]d: return %[3]sOpternUint%[4]dx%[5]d\n", w, c, splitOpPrefix, wt, ct)
	}, out)
...
func classifyBooleanSIMD(v *%sValue) SIMDLogicalOP {
	switch v.Op {
		case `, splitCorePrefix)
	intShapes.forAllShapes(func(seq int, t, upperT string, w, c int, out io.Writer) {
		op := "And"
		if seq > 0 {
			fmt.Fprintf(out, ",%sOp%s%s%dx%d", splitOpPrefix, op, upperT, w, c)
		} else {
			fmt.Fprintf(out, "%sOp%s%s%dx%d", splitOpPrefix, op, upperT, w, c)
		}
		seq++
...
`)
	...
	intShapes.forAllShapes(
		func(seq int, t, upperT string, w, c int, out io.Writer) {
			fmt.Fprintf(out, "case %sOpXor%s%dx%d: ", splitOpPrefix, upperT, w, c)
			fmt.Fprintf(out, `
				if y := v.Args[1]; y.Op == %sOpEqual%s%dx%d &&
				   y.Args[0] == y.Args[1] {
				   		return sloNot
				}
				`, splitOpPrefix, upperT, w, c)
