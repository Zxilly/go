...
func (s *state) exprCheckPtr(n ir.Node, checkPtrOK bool) *ssa.Value {
	...
	case ir.OCFUNC:
		...
		if buildcfg.GOARCH == "wasm32" {
			// wasm32 function values contain a compact call_indirect handle,
			// not the function's logical PC. Load it from the canonical
			// function-value symbol so data and dynamic closures agree.
