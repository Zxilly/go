...
type regState struct {
	v *ssa.Value // Original (preregalloc) Value stored in this register.
	c *ssa.Value // A Value equal to v which is currently in a register.  Might be v or a copy of it.
...
type endReg struct {
	r ssaop.Register
	v *ssa.Value // pre-regalloc value held in this register (TODO: can we use ID here?)
	c *ssa.Value // cached version of the value
}

type startReg struct {
	r   ssaop.Register
	v   *ssa.Value // pre-regalloc value needed in this register
	c   *ssa.Value // cached version of the value
...
// v's original value.
func (s *regAllocState) setOrig(c *ssa.Value, v *ssa.Value) {
...
// r must be unused.
func (s *regAllocState) assignReg(r ssaop.Register, v *ssa.Value, c *ssa.Value) {
	if s.f.Pass.Debug > ssa.RegDebug {
...
func (s *regAllocState) allocReg(mask ssaop.RegMask, v *ssa.Value) ssaop.Register {
	if v.OnWasmStack {
		return noRegister
	}
...
func (s *regAllocState) makeSpill(v *ssa.Value, b *ssa.Block) *ssa.Value {
	vi := &s.values[v.ID]
	if vi.Spill != nil {
...
func (s *regAllocState) allocValToReg(v *ssa.Value, mask ssaop.RegMask, nospill bool, pos src.XPos) *ssa.Value {
	...
	if !vi.Regs.Empty() {
		...
		if s.f.Pass.Debug > ssa.LogSpills {
...
func (s *regAllocState) init(f *ssa.Func) {
	...
	if cap(s.f.Cache.RegallocValues) >= nv {
		...
		s.f.Cache.RegallocValues = make([]ssa.ValState, nv)
	}
...
func (s *regAllocState) addUse(id ssa.ID, dist int32, pos src.XPos) {
	r := s.freeUseRecords
	if r != nil {
...
func (s *regAllocState) advanceUses(v *ssa.Value) {
...
// a call instruction.
func (s *regAllocState) dropIfUnused(v *ssa.Value) {
...
// current instruction.
func (s *regAllocState) liveAfterCurrentInstruction(v *ssa.Value) bool {
...
func (s *regAllocState) regspec(v *ssa.Value) ssaop.RegInfo {
	op := v.Op
	...
	if op == ssaop.OpArgIntReg {
		...
		return ssaop.RegInfo{Outputs: []ssaop.OutputInfo{{Regs: ssa.RegMaskAt(ssaop.Register(reg))}}}
	}
	...
	if op.IsCall() {
		if ac, ok := v.Aux.(*ssa.AuxCall); ok && ac.RegCache != nil {
...
var tmpVal ssa.Value

func (s *regAllocState) regalloc(f *ssa.Func) {
	...
	desiredSecondReg := map[ssa.ID][4]ssaop.Register{} // desired register allocation for 2nd part of a tuple
	...
	for _, b := range s.visitOrder {
		...
		for idx, v := range oldSched {
			...
			if v.Op == ssaop.OpSelect0 || v.Op == ssaop.OpSelect1 || v.Op == ssaop.OpSelectN {
				if s.values[v.ID].NeedReg {
					if v.Op == ssaop.OpSelectN {
						s.assignReg(ssaop.Register(s.f.GetHome(v.Args[0].ID).(ssa.LocResults)[int(v.AuxInt)].(*ssabase.Register).Num), v, v)
					} else {
						var i = 0
						if v.Op == ssaop.OpSelect1 {
							i = 1
						}
						s.assignReg(ssaop.Register(s.f.GetHome(v.Args[0].ID).(ssa.LocPair)[i].(*ssabase.Register).Num), v, v)
					}
				}
				b.Values = append(b.Values, v)
				s.advanceUses(v)
			...
			// in place, keep it.
			args = append(args[:0], make([]*ssa.Value, len(v.Args))...)
		...
		ok:
			...
			// Pick registers for outputs.
			{
				...
				if v.Type.IsTuple() {
					...
					outLocs := make(ssa.LocResults, maxOutIdx+1, maxOutIdx+1)
				...
				if tmpReg != noRegister {
					...
					if s.f.TempRegs == nil {
						s.f.TempRegs = map[ssa.ID]*ssabase.Register{}
					}
		...
		controls := append(make([]*ssa.Value, 0, 2), b.ControlValues()...)
...
type edgeState struct {
	s    *regAllocState
	p, b *ssa.Block // edge goes from p->b.
...
type contentRecord struct {
	vid   ssa.ID     // pre-regalloc value
	c     *ssa.Value // cached value
	final bool       // this is a satisfied destination
	pos   src.XPos   // source position of use of the value
}

type dstRecord struct {
	loc    ssa.Location // register or stack slot
	vid    ssa.ID       // pre-regalloc value it should contain
	splice **ssa.Value  // place to store reference to the generating instruction
	pos    src.XPos     // source position of use of this location
}

// setup initializes the edge state for shuffling.
func (e *edgeState) setup(idx int, srcReg []endReg, dstReg []startReg, stacklive []ssa.ID) {
...
func (e *edgeState) processDest(loc ssa.Location, vid ssa.ID, splice **ssa.Value, pos src.XPos) bool {
	pos = pos.WithNotStmt()
	...
	if c == nil || e.s.values[vid].Rematerializeable {
		...
		if dstReg {
			...
			x = e.p.NewValue1(pos, ssaop.OpStoreReg, loc.(ssa.LocalSlot).Type, x)
		}
	} else {
		...
		if srcReg {
			if dstReg {
				...
				x = e.p.NewValue1(pos, ssaop.OpStoreReg, loc.(ssa.LocalSlot).Type, c)
			}
		} else {
			if dstReg {
				...
				x = e.p.NewValue1(pos, ssaop.OpStoreReg, loc.(ssa.LocalSlot).Type, t)
			}
		}
	}
...
func (e *edgeState) erase(loc ssa.Location) {
	cr := e.contents[loc]
	if cr.c == nil {
		return
	}
...
func (e *edgeState) findRegFor(typ *types.Type) ssa.Location {
		...
		for _, c := range a {
			if r, ok := e.s.f.GetHome(c.ID).(*ssabase.Register); ok && m.HasReg(ssaop.Register(r.Num)) {
				if !c.Rematerializeable() {
					...
					t := ssa.LocalSlot{N: e.s.f.NewLocal(c.Pos, c.Type), Type: c.Type}
...
func (s *regAllocState) computeLive() {
	...
	var loopLiveIn map[*ssa.Loop][]liveInfo
	var numCalls []int32
	if len(s.loopnest.Loops) > 0 && !s.loopnest.HasIrreducible {
		loopLiveIn = make(map[*ssa.Loop][]liveInfo)
	...
	slices.SortFunc(loops, func(a, b *ssa.Loop) int {
...
func (s *regAllocState) computeDesired() {
	...
	for {
		...
		for _, b := range po {
			...
			var headerLoop *ssa.Loop // loop whose header is b, if any
			...
			for ; i >= 0; i-- {
				...
				for _, r := range prefs {
					if r != noRegister {
						desired.avoid = desired.avoid.Minus(ssa.RegMaskAt(r))
					}
				}
...
func updateLive(t *ssa.SparseMapPos, live []liveInfo) []liveInfo {
	live = live[:0]
	if cap(live) < t.Size() {
...
// likeliness
func branchDistance(b *ssa.Block, s *ssa.Block) int32 {
	if len(b.Succs) == 2 {
		if b.Succs[0].B == s && b.Likely == ssa.BranchLikely ||
			b.Succs[1].B == s && b.Likely == ssa.BranchUnlikely {
			return likelyDistance
		}
		if b.Succs[0].B == s && b.Likely == ssa.BranchUnlikely ||
			b.Succs[1].B == s && b.Likely == ssa.BranchLikely {
...
func (s *regAllocState) debugPrintLive(stage string, f *ssa.Func, live [][]liveInfo, desired []desiredState) {
...
func (s *regAllocState) debugPrintLiveBlock(b *ssa.Block, live []liveInfo, desired *desiredState) {
	fmt.Printf("  %s:", b)
...
func (d *desiredState) get(vid ssa.ID) [4]ssaop.Register {
...
func (d *desiredState) add(vid ssa.ID, r ssaop.Register) {
...
func (d *desiredState) addList(vid ssa.ID, regs [4]ssaop.Register) {
...
func (d *desiredState) remove(vid ssa.ID) [4]ssaop.Register {
	for i := range d.entries {
