...
func pcSlotCount(size int64) uint64 {
	if size < 1 {
		return 1
	}
	return uint64(size)
...
// wasm32 continuation occupies an existing 8-byte PC slot: its low word is
...
func assignAddress(arch *sys.Arch, ldr *loader.Loader, sect *sym.Section, n int, s loader.Sym, va uint64, isTramp bool) (*sym.Section, int, uint64) {
	...
	//
	// On GOARCH=wasm, s.Value is still 1<<63 + PC_F<<16. On wasm32, s.Value
	// is a dense logical PC token. Each function owns SymSize(s) consecutive
	// tokens, so s.Value+PC_B identifies a resume point. A separate compact
	...
	size := pcSlotCount(ldr.SymSize(s))
...
// tableEntries returns the defined-function index for each indirect-call
// table slot. handles is indexed by final defined-function index and contains
// the handles assigned before text layout. wasm32 text layout may reorder
// functions, so its table must be permuted back into handle order.
func tableEntries(arch *sys.Arch, handles []uint32) ([]uint32, error) {
	entries := make([]uint32, len(handles))
	if arch.PtrSize == 8 {
		for i := range entries {
			entries[i] = uint32(i)
		}
	...
	for funcIndex, handle := range handles {
		if handle == 0 || handle > uint32(len(handles)) {
			return nil, fmt.Errorf("invalid wasm32 function handle %d for defined function %d", handle, funcIndex)
		}
		slot := handle - 1
		if seen[slot] {
			return nil, fmt.Errorf("duplicate wasm32 function handle %d", handle)
		}
		seen[slot] = true
		entries[slot] = uint32(funcIndex)
	}
	return entries, nil
}

func indirectTableOffset(arch *sys.Arch) uint32 {
	if arch.PtrSize == 4 {
		return 1 // zero is the nil function handle
	}
	return 0
}
...
func asmb2(ctxt *ld.Link, ldr *loader.Loader) {
	...
	handles := make([]uint32, len(fns))
	if ctxt.Arch.PtrSize == 4 {
		for i, fn := range ctxt.Textp {
			handle, ok := ctxt.WasmFuncIndex[fn]
			if !ok {
				ldr.Errorf(fn, "missing wasm32 function handle")
				continue
			}
			handles[i] = handle
		}
	}
	table, err := tableEntries(ctxt.Arch, handles)
	if err != nil {
		ld.Errorf("cannot build WebAssembly function table: %v", err)
		table = make([]uint32, len(fns))
	}
	...
	tableOffset := indirectTableOffset(ctxt.Arch)
	writeTableSec(ctxt, int(tableOffset)+len(table))
	writeMemorySec(ctxt, ldr)
	writeGlobalSec(ctxt)
	writeExportSec(ctxt, ldr, len(hostImports), funcIndexes)
	writeElementSec(ctxt, uint64(len(hostImports)), tableOffset, table)
...
// wasm32 starts at table slot 1 so zero remains the nil function handle.
func writeElementSec(ctxt *ld.Link, numImports uint64, tableOffset uint32, entries []uint32) {
	...
	writeUleb128(ctxt.Out, uint64(len(entries))) // number of entries
